...
| `media_convert` | `ROBOLECTRIC` | `MediaConvertToolTest`, `FfmpegCommandTest` — exhaustive coverage of the generated ffmpeg command on the JVM — POSIX shell quoting against spaces, embedded quotes and metacharacters, /storage/emulated/0 to /sdcard translation including secondary-user roots and near-miss prefixes, bitrate validation, and every audio target's codec arguments — plus the guards that run before Termux is invoked: unsupported and video output formats, nonsense bitrates, paths inside Gotcha's private storage that Termux cannot see, missing input, same-file conversion, existing-output refusal and directory outputs. The conversion itself is device-only and additionally needs Termux with the ffmpeg package installed, so a real MP3 round trip must be verified by hand |
| `media_edit` | `ROBOLECTRIC` | `MediaEditToolTest`, `MediaTimeSpecTest` — timecode parsing and trim-window arithmetic on the JVM (m:ss, h:mm:ss, unit and plain-seconds forms, ambiguous-field and backwards-window refusals, clamping an over-long end), plus every guard that runs before a codec is touched: unknown operation, missing arguments, trim with no bounds, refusal to write back over the input or over any concat input, existing-output refusal, directory output, the MP4/M4A container check and the .mp3 redirect to .m4a, speed bounds and the pointless speed=1.0 re-encode, non-positive compress height, and concat's input-count limits. The export itself is device-only — Media3 Transformer drives MediaCodec, which is native and has no Robolectric shadow, so the output of trim, extract_audio, remove_audio, compress, speed and concat must be verified by hand on hardware |
...
| `pull_from_termux` | `ROBOLECTRIC` | `TermuxFileBridgeTest` — TermuxFileBridgeTest — the deterministic bridge that moves a file out of Termux: ~/ and $HOME expansion to Termux's home, the exact python3 loopback sender script (ephemeral port, single-use nonce, READY handshake, 64KiB chunked stream, nonce mismatch rejection, python-quoting of the path), and the shared-storage-vs-loopback selection (only /sdcard when the ~/storage link exists AND the destination is reachable from Termux). The live transfer — a real cp through /sdcard or a loopback python3 send on a device with Termux — must be verified by hand |
...
| `synthesize_podcast_dialogue` | `ROBOLECTRIC` | `PodcastToolTest`, `PodcastAudioTest` — shares synthesize_podcast's pipeline and its coverage, plus the dialogue-specific guards and mechanics: unknown-speaker refusal, empty-dialogue refusal, blank turns dropped after sanitising rather than synthesized as dead air, and the three-level host-voice resolution (explicit argument beats configured setting beats the first voice in the model's list that differs from host A). Pacing is covered on both halves: pause resolution (a line's pause_ms beats gap_ms beats the 300ms default, zero honoured as zero rather than treated as unset, absurd or negative values clamped) and the silence append itself (duration grows by exactly the requested pause, the pause inherits the speech's own sample rate and channel count, the spoken bytes survive untouched, the appended region is genuinely silent, and a padded WAV re-parses so it can be padded again). The audible result — two distinguishable hosts and whether the chosen pauses actually sound like conversation — is device-only and must be verified by hand |
...
| `transcribe_file` | `ROBOLECTRIC` | `TranscribeToolTest` — every guard that runs before the upload — Android/unconfigured STT provider refusals with directions to Settings, missing/empty/oversized files, unsupported extensions refused before the backend is touched — plus the properties that make the tool safe to point at a user's only copy of a recording: the input file demonstrably survives transcription byte-for-byte (the deliberate contrast with SttEngine.transcribeApi, which deletes its input), model resolution (explicit argument beats configured setting beats the first STT model the API advertises), empty transcripts reported as errors rather than silence, and over-long transcripts truncated with an honest note. The upload itself needs a reachable STT endpoint, so a real memo round trip is device-only manual verification |
